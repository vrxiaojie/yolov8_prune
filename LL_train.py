...
root = os.getcwd()
## 配置文件路径
name_yaml             = os.path.join(root, "data.yml")
name_pretrain         = os.path.join(root, "yolov8s.pt")
## 原始训练路径
path_train            = os.path.join(root, "weights")
name_train            = os.path.join(path_train, "best.pt")
## 约束训练路径、剪枝模型文件
path_constraint_train = os.path.join(root, "runs/detect/Constraint")
...
## 微调路径
path_fineturn         = os.path.join(root, "runs/detect/finetune")
...
def step1_train():
    model = YOLO(name_pretrain)
    model.train(data=name_yaml, device="0", imgsz=640, epochs=50, batch=32, workers=0, save_period=1, name=path_train)  # train the model

## 2024.3.4添加【amp=False】
def step2_Constraint_train():
    model = YOLO(name_train)
    model.train(data=name_yaml, device="0", imgsz=640, epochs=50, batch=32, amp=False, workers=0, save_period=1,name=path_constraint_train)  # train the model
